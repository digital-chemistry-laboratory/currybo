...
from .utility_function import BaseMCUtilityFunction, UncertaintyUtility, QuantitativeImprovement, SimpleRegret
...
class SequentialAcquisition(BaseMCAcquisitionStrategy):
    ...
    def get_recommendation(
        ...
        x_acqf = MCAcquisitionFunction(
            model=model,
            utility=self._x_utility_type(maximize=self.maximization, **self._x_utility_kwargs),
            aggregation_function=self.aggregation_function,
            sample_reduction=self.sample_reduction,
            q_reduction=self.q_reduction,
            w_options=self.w_options,
        ...
        next_x, _ = x_acqf.optimize(q=1, bounds=self.x_bounds, options=self.x_options, **kwargs)

        # ATTN: The following lines currently only work for q = 1 and single-output models (m = 1)

        w_utility = self._w_utility_type(maximize=self.maximization, **self._w_utility_kwargs)
        ...
        if self.w_options.shape[0] * self.x_options.shape[0] > train_x.shape[0]:
            ...
            # Loop to avoid selecting (x,w) pairs that have already been evaluated or recommended
            # For proposed x, remove already evaluated ws first
            # If all ws have been evaluated for proposed x, re-sample x and remove original x from options
            ...
            scores = w_acqf_values.clone()
            scores[matched_idx] = -float('inf')
            next_w_idx = scores.argmax().item()

            x_options = self.x_options
            while len(matched_idx) == all_xw.shape[0]:
                x_options = x_options[~torch.isclose(x_options, next_x, rtol=rtol).all(dim=1)]
                next_x, _ = x_acqf.optimize(q=1, bounds=self.x_bounds, options=x_options, **kwargs)
                all_xw, all_xw_idx = create_all_permutations(next_x, self.w_options)
                posterior = model.posterior(all_xw.unsqueeze(dim=1))
                mc_samples = sampler(posterior)  # num_mc_samples x n x q=1 x m=1
                w_utility_values = w_utility(mc_samples.squeeze(-1))  # num_mc_samples x n x q=1
