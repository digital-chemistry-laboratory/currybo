...
from currybo.acquisition_strategies.utility_function import BaseMCUtilityFunction, UncertaintyUtility, QuantitativeImprovement
from currybo.surrogate_models import BaseSurrogate
from currybo.aggregation_functions import BaseAggregation, Mean
from .utility_function import SimpleRegret
...
class JointLookaheadAcquisition(BaseMCAcquisitionStrategy, torch.nn.Module):
    ...
    def get_recommendation(
        ...
        if kwargs['cli_args']['utility'] in ["QualitativeImprovement", "QuantitativeImprovement"]:
            greedy_acqf = MCAcquisitionFunction(
                model=model,
                utility=SimpleRegret(maximize=self.maximization, **self._x_utility_kwargs),
            ...
            best_f, _ = greedy_acqf.optimize(q=1, bounds=self.x_bounds, options=self.x_options, **kwargs)
        ...
        train_x = torch.cat((model._original_train_inputs, model.train_inputs[0][model._original_train_inputs.shape[0]:]), dim=0)

        if self.w_options.shape[0] * self.x_options.shape[0] > train_x.shape[0]:

            rtol = 1e-5
            isclose = torch.isclose(all_xw[:, None, :], train_x[None, :, :], rtol=rtol)
            row_pair_close = isclose.all(dim=-1)
            matched_any = row_pair_close.any(dim=1)
            matched_idx = matched_any.nonzero(as_tuple=False).flatten()
            scores = all_acqf_values.clone()
            scores[matched_idx] = -float('inf')
            next_x_idx, next_w_idx = all_xw_idx[torch.argmax(scores)]
            next_x = self.x_options[next_x_idx].unsqueeze(0)
...
class SequentialLookaheadAcquisition(BaseMCAcquisitionStrategy, torch.nn.Module):
    ...
    def get_recommendation(
        ...
        if kwargs['cli_args']['x_utility'] in ["QualitativeImprovement", "QuantitativeImprovement"]:
            greedy_acqf = MCAcquisitionFunction(
                model=model,
                utility=SimpleRegret(maximize=self.maximization, **self._x_utility_kwargs),
                aggregation_function=self.aggregation_function,
        ...
        all_acqf_values_w = torch.cat(
            [self._forward_w(X_.unsqueeze(1)) for X_ in all_xw.split(self._inference_batch_size)],
        )
        _, next_w_idx = all_xw_idx[torch.argmax(all_acqf_values_w)]
        ...
        if self.w_options.shape[0] * self.x_options.shape[0] > train_x.shape[0]:
            ...
            matched_any = row_pair_close.any(dim=1)
            matched_idx = matched_any.nonzero(as_tuple=False).flatten()
            scores = all_acqf_values_w.clone()
            scores[matched_idx] = -float('inf')
            next_w_idx = scores.argmax()

            measured_idx = torch.tensor([], dtype=torch.long)
            i = 0
            while len(matched_idx) == all_xw.shape[0]:
                i += 1
                idx = torch.isclose(self.x_options, next_x, rtol=rtol).all(dim=1).nonzero(as_tuple=False).flatten()
                if idx.item() not in measured_idx.tolist():
                    measured_idx = torch.cat([measured_idx, idx.to(dtype=torch.long)])
                scores_x = all_acqf_values.clone()
                scores_x[measured_idx.long()] = -float('inf')
                next_x = self.x_options[torch.argmax(scores_x)].unsqueeze(0)
